 ...
 * @since 13/11/20
 */
public class InnerTableDeserializer extends StdDeserializer<InnerTableSchema> {

  public InnerTableDeserializer() {
    super(InnerTableSchema.class);
  }


  @Override
  public InnerTableSchema deserialize(JsonParser jsonParser,
      DeserializationContext ctxt)
      throws IOException {
    JsonNode node = jsonParser.getCodec().readTree(jsonParser);
    List<Object> columns = readSchemaColumns(node, jsonParser, ctxt);
    return new InnerTableSchema(readStringProperty(node, "id"),
        ...
        Source.valueOf(readStringProperty(node, "source")),
            columns, readStringProperty(node, "aliasName"));
  }
