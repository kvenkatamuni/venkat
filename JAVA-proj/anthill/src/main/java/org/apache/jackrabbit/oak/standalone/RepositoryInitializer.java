...
public class  RepositoryInitializer {
    ...
    @Value("${oak.mongo.username}")
    private String mongoUserName;

    @Value("${oak.mongo.port}")
    private String port;

    @Value("${oak.mongo.server}")
    private String server;


    @Value("${oak.mongo.password}")
    ...
    @Value("${mongoCluster.enabled}")
    Boolean mongoClusterEnabled;

    @Value("${enableLeaseCheck:false}")
    Boolean enableLeaseCheck;
    ...
    public void initialize() throws Exception {
        System.setProperty("oak.mongo.uri",getMongoURI());
        initRepository();
    }
    ...
    private void initRepository() throws IOException, RepositoryException {
        ...
        LeaseCheckMode leaseCheckMode = enableLeaseCheck ? LeaseCheckMode.STRICT
    ...
    private String getMongoURI() {
        ...
        return createMongoUri();
    }

    private String createMongoUri(){
        String prefix = mongoClusterEnabled ? "mongodb+srv://" : "mongodb://";
        String s = new StringBuilder(prefix)
                .append(mongoUserName)
                .append(":")
                .append(SentryServiceImpl.getSecret(sentryUrl,passwordToken))
                .append("@")
                .append(server)
                .toString();

        if(mongoClusterEnabled){
            return s;
        }else {
            String connectionString =new StringBuilder(s)
                    .append(":")
                    .append(port)
                    .toString();
            if(sslEnabled){
                return new StringBuilder(connectionString)
                        .append("/?ssl=true")
                        .toString();
            }
